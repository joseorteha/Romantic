...
// Código secreto basado en la fecha importante: 11-01-2023
        ...
        if (passcode.length < 6) {
            ...
            if (newPasscode.length === 4) {
                ...
                if (enteredPasscode === CORRECT_PASSCODE) {
                    setMessage('¡Correcto, mi amor! ❤️');
                    setTimeout(() => {
                        setMessage('Redirigiendo... 🌟');
                        navigate("/question");
                    }, 500);
                } else {
                    setMessage('Código incorrecto. Pista: nuestra fecha especial 📅');
                    setTimeout(() => {
                        setPasscode([]);
                        setMessage('');
                    }, 4000);
    ...
    return (
        <div className="min-h-screen w-full bg-black/60 backdrop-blur-sm text-white flex items-center justify-center overflow-hidden">
            <div className="flex flex-col items-center max-w-full">
                {/* Título */}
                <h1 className="text-2xl font-light mb-8">Ingresa el código secreto</h1>
                ...
                {/* Mensaje */}
                {message && (
                    <div className={`mb-4 -mt-9 text-sm font-bold ${message.includes('Correcto') ? 'text-green-500' : 'text-red-500'}`}>
                        {message}
                    </div>
                )}
