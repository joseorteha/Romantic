...
function Question() {
  ...
  // Búsquedas populares personalizadas
  const trends = [
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (
      searchQuery.toLowerCase() === 'cuánto tiempo llevamos juntos?' ||
      searchQuery.toLowerCase() === 'cuánto tiempo llevamos juntos'
    ) {
      navigate('/timer');
    } else {
      ...
      {!isInputFocused ? (
        <div className="flex flex-col items-center px-4 pt-16">
          ...
          <div className="mt-8 w-full max-w-[600px]">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-2xl">Búsquedas populares</h2>
        ...
        <div className="flex flex-col min-h-screen w-full max-w-[480px] mx-auto">
          ...
          <div className="mt-10 px-5 text-white/50">
            <p>Prueba buscando "Cuánto tiempo llevamos juntos?" ❤️</p>
          </div>
        </div>
      )}
    </div>
